...
interface ItemDao {
    ...
    @Query("SELECT * From ItemEntity, PriceEntity WHERE PriceEntity.barcode = :barcode")
    suspend fun getItemEntitiesByBarcode(barcode : String) : List<ItemWithPricesEntity>

    @Query("SELECT * FROM ItemEntity")
    fun getAllItemEntity() : Flow<List<ItemWithPricesEntity>>

    @Query("UPDATE ItemEntity SET image_id = :imageId WHERE id = :itemId")
    suspend fun updateItemEntityImageId(itemId : Long, imageId : Long)
